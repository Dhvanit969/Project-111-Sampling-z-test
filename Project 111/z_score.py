...
# creating the loop of random from 0 to 100 ad setting random mean to 30 values etc
...
# function to get the mean of 100 sets of data
mean_list = []
...
#  finding the mean of the students who work hard on maths lab and bar graph
...
# #finding the mean of the students use the app for their practice and plotting it on the plot.
...
fig = ff.create_distplot([mean_list], ["student marks"], show_hist=False)
fig.add_trace(go.Scatter(x=[mean, mean], y=[0, 1.4], mode="lines", name="MEAN"))
fig.add_trace(go.Scatter(x=[mean_of_sample3, mean_of_sample3], y=[0, 1.4], mode="lines", name="MEAN OF STUDNETS WHO WERE ENFORCED WITH MATH REGISTERS"))
fig.add_trace(go.Scatter(x=[second_std_deviation_end, second_std_deviation_end], y=[0, 1.4], mode="lines", name="STANDARD DEVIATION 2 END"))
fig.add_trace(go.Scatter(x=[third_std_deviation_end, third_std_deviation_end], y=[0, 1.4], mode="lines", name="STANDARD DEVIATION 3 END"))
fig.show()
